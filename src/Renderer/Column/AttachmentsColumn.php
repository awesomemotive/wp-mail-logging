        ...
        foreach ( $attachments as $attachment ) {
            ...
            if ( ! empty( $attachment ) ) {
                ...
                if ( is_file( $attachment_path ) ) {
                    $attachment_append .= '<a target="_blank" href="' . esc_url( $attachment_url ) . '" title="' . esc_attr( $filename ) . '">' . WPML_Utils::generate_attachment_icon( $attachment_path ) . '</a> ';
                } else {
                    ...
                    $attachment_append .= '<i class="fa fa-times" title="' . esc_attr( $message ) . '"></i>';
                }
            }
        }
    ...
    function column_attachments( $item ) {
        ...
        foreach ( $attachmentRelPaths as $attachmentRelPath ) {
            ...
            if ( !$attachment->isGone() ) {
                $attachment_append .= '<a target="_blank" href="' . esc_url( $attachment->getUrl() ) . '" title="' . esc_attr( $attachment->getFileName() ) . '">' . WPML_Utils::generate_attachment_icon( $attachment ) . '</a> ';
            } else {
