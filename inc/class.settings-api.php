...
class WeDevs_Settings_API {
    ...
    function set_fields( $fields ) {
        ...
        foreach ($fields as $section => $options) {
        	$filtered_fields[$section] = array();
        	foreach ($options as $option) {
        		
	        	if( isset($option['name'] ) && !empty( $option['name'] ) ) {
	        		$fieldname = $option['name'];
	        		$filtered_fields[$section][] = $option = apply_filters("settings_filter_{$fieldname}", $option);
	        		
	        	} else {
	        		$filtered_fields[$section][] = $option;
	        	}
        	}
        }
        
        $this->settings_fields = $filtered_fields;
     ...
     * @param bool    $default_definition use default from initialization
     * @return string
     */
    function get_option( $option, $section, $default = '', $default_definition = false ) {

        $options = get_option( $section );
		
        if ( isset( $options[$option] ) ) {
            return $options[$option];
        } else if($default_definition) {
	        foreach( $this->settings_fields as $section => $all_options ) {
	        	foreach ( $all_options as $config_option ) {
	        		if ( $config_option['name'] == $option ) {
	        			if( isset( $config_option['default'] ) && !empty($config_option['default']) ) {
