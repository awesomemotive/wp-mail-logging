...
class WPML_Email_Log_List_Test extends \PHPUnit_Framework_TestCase {
    ...
    public function test_column_overridden_timestamp() {
        $expected = '2018-09-24 16:02:11';
        $this->assertEquals($expected, $this->logListTable->column_overridden_timestamp($this->item));
    }

    public function test_column_overridden_error_empty() {
        $expected = '';
        $this->item['error'] = "";
        $this->assertEquals($expected, $this->logListTable->column_overridden_error($this->item));
    }

    public function test_column_overridden_error_array() {
        $expected = "<i class='fa fa-exclamation-circle' title='a,b' aria-hidden='true'></i>";
        $this->assertEquals($expected, $this->logListTable->column_overridden_error($this->item));
    }

    public function test_render_mail() {
        $expected = '<span class="title">Time: </span>2018-09-24 16:02:11<span class="title">Receiver: </span>example@exmple.com<span class="title">Subject: </span>Test<span class="title">Message: </span>&lt;b&gt;Bold&lt;/b&gt;<span class="title">Headers: </span>From: &quot;admin&quot; ,\nCc: example2@example.com,\nReply-To: admin <span class="title">Attachments: </span><span class="title">Error: </span>&lt;i class=\'fa fa-exclamation-circle\' title=\'a,b\'&gt;&lt;/i&gt;';
